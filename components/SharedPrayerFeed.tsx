 ...
 *  · scope="coach"         — what the coach's disciples shared with them.
 *  · scope="group"         — what fellow Grace Group members shared.
 *  · scope="constellation" — what's shared with everyone (GBC).
...
export default function SharedPrayerFeed({
  ...
  const S = subtitle ?? (
    scope === 'coach' ? 'Requests and praises your disciples shared with you'
  ...
  return (
    <section className="cn-card mb-6 p-4">
      <div className="flex items-center gap-3">
        ...
        {unread > 0 && (
          <span className="rounded-full px-2 py-0.5 text-[10px] font-bold" style={{ background: '#9B80FF', color: '#fff' }}>
            {unread} new
          </span>
        )}
        {items.length > 0 && (
