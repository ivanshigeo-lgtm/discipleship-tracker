...
export default function PersonProfileModal({ person, initialTab = 'profile', onClose, onSaved, onDeleted, onPersonCreated, onOpenEngagement }: PersonProfileModalProps) {
  ...
  // Just the login page — never a way into their account. They sign in, or use
  // "Forgot password?" to reset via their own email. No one (not even a coach)
  // can access their account or their private SOAPs.
  const copyLoginLink = () => {
          ...
          {person.auth_user_id && (
            <div className="mt-3 flex flex-wrap items-center gap-2 rounded-lg bg-[rgba(54,214,195,.1)] px-3 py-2">
              ...
              <button
                ...
                title="Copy the login link to send them. They sign in, or use “Forgot password?” to reset via their own email."
                className="ml-auto rounded-full bg-[var(--establish)] px-2.5 py-1 text-[10px] font-semibold text-[var(--void)] transition-opacity hover:opacity-90"
              >
                {accessState === 'copied' ? 'Copied!' : 'Copy login link'}
              </button>
            </div>
          )}
        </div>
