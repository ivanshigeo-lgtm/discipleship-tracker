...
describe('lastOfSeriesById — every finite engagement meeting_type', () => {
  ...
  for (const meetingType of ['One2One', 'Making Disciples', 'Church Community', 'Empowering Leaders', 'Coffee', null]) {
    it(`flags the last pending ${meetingType ?? 'untyped'} series`, () => {
      const rows = saturdays.map((d, i) =>
        e({ id: `${meetingType ?? 'none'}-${i}`, follow_up_date: d, meeting_type: meetingType, description: meetingType ?? 'Hangout' }),
      )
      const map = lastOfSeriesById(rows, 'ivan')
      assert.equal(map.size, 1)
  ...
  it('flags a production-style ONe2ONe description with null series_id', () => {
    const dates = ['2026-08-04', '2026-08-13', '2026-08-27', '2026-09-03', '2026-09-10', '2026-09-17']
    const rows = dates.map((d, i) =>
      e({ id: `j${i}`, follow_up_date: d, description: 'ONe2ONe 1', meeting_type: 'One2One', follow_up_time: '17:00' }),
    )
    const map = lastOfSeriesById(rows, 'ivan')
    assert.equal(map.size, 1)
    assert.equal(map.get('j5')?.lastDate, '2026-09-17')
    assert.equal(map.get('j5')?.occurrenceCount, 6)
    // A 7-day agenda that only holds this week's row would miss the last date.
    const inWindow = rows.filter(r => r.follow_up_date! <= '2026-08-10')
    assert.equal(inWindow.some(r => map.has(r.id)), false)
  })

  it('flags a finite Church Community series (not a standing victory_group)', () => {
  ...
  it('does not mix two meeting types that share a person and time', () => {
    const md = saturdays.map((d, i) =>
      e({ id: `md${i}`, follow_up_date: d, meeting_type: 'Making Disciples', description: 'Making Disciples' }),
    )
    const coffee = saturdays.map((d, i) =>
      e({ id: `cf${i}`, follow_up_date: d, meeting_type: 'Coffee', description: 'Coffee' }),
    )
  ...
  it('splits a long pause into a new series instead of gluing unbounded history', () => {
    const rows = [
    ...
    const map = lastOfSeriesById(rows, 'ivan')
    assert.equal(map.size, 2)
    assert.ok(map.get('old2'))
    assert.ok(map.get('new3'))
  })
})
