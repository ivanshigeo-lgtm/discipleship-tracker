-- Re-point the two assessment-onramp tables (added after merge_people was
-- written) so merging a lead who took the public assessment no longer trips
-- the FK safety-sweep. onramp_submissions: straight repoint (person_id not
-- unique). assessment_progress: PK is person_id, so dedupe then repoint.
...
begin
  ...
  -- assessment_progress PK is person_id (one row/person): drop the dup's row if
  -- the keeper already has progress, then repoint
  delete from assessment_progress a
    where a.person_id = p_dup
      and exists (select 1 from assessment_progress x where x.person_id = p_keep);
  update assessment_progress set person_id = p_keep where person_id = p_dup;
  ...
  update onramp_submissions set person_id = p_keep where person_id = p_dup;
